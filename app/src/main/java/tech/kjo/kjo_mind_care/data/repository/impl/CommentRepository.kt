        ...
        val subscription = getCommentsCollection(blogId)
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                if (snapshot != null) {
                    ...
                    launch {
                        val currentUserUid = authRepository.getCurrentUserUid() // Call suspend function here
                        val comments = snapshot.documents.mapNotNull { document ->
                            try {
                                document.toObject(Comment::class.java)?.copy(id = document.id)?.apply {
                                    isMine = author.uid == currentUserUid
                                }
                            } catch (e: Exception) {
